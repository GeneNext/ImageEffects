Shader "Hidden/BokehEffect"
{
	...
	float4 frag_outputDepth(v2f i) : COLOR
	{
		...
		return abs(_FocalDistance - Linear01Depth(UNITY_SAMPLE_DEPTH(color)));
	}

	float4 frag_outputBlend(v2f i) : COLOR
	{
		float depth = tex2D(_DepthTex, i.uv).r * 1;

		float4 mainTex = tex2D(_MainTex, i.uv);
		float4 bokehTex = tex2D(_BokehTex, i.uv);
		//return lerp(mainTex, bokehTex, depth);
		//if(depth	< 0.01)
		//{
		//	return mainTex;
		//}
		...
		//{
			return bokehTex;
		//}
		return lerp(mainTex, bokehTex, 1);
		return 1 - depth;
	}
