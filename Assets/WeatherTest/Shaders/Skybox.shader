...
Properties {
	...
	_CloudCube("Cloud Cube", CUBE) = "cube" {}
	_CloudFactor("Cloud Factor", Range(0, 2)) = 0
	_CloudSpeed("Cloud Speed", Range(-1, 1)) = 1
}
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                col += IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray) * _TintColor;
            }
        #endif
			float4 cloud = 0;
			float3 viewDir = normalize(UnityWorldSpaceViewDir(IN.worldPos));
			float3 rotated1 = RotateAroundYInDegrees(-viewDir, _Time.z * _CloudSpeed * 0.5);
			float3 reflectDir1 = normalize(reflect(rotated1, IN.worldNormal));
			float4 cloud1 = texCUBE(_CloudCube, reflectDir1);
			float3 rotated2 = RotateAroundYInDegrees(-viewDir, _Time.z * _CloudSpeed * 3);
			float3 reflectDir2 = normalize(reflect(rotated2, IN.worldNormal));
			float4 cloud2 = texCUBE(_CloudCube, reflectDir2);
			//cloud = pow(cloud, 2);
			//cloud = lerp(cloud1, cloud2, Luminance(cloud2.rgb)) * _CloudFactor;
			cloud = (cloud1 + cloud2) * _CloudFactor;
			cloud *= _Exposure * saturate(1 - y);
			//cloud = saturate(cloud);
			//cloud.rgb = lerp(cloud.rgb, GammaToLinearSpace(cloud.rgb), 0.5);

			//col = col;
			col = lerp(col, cloud, Luminance(cloud.rgb)) * _TintColor + _AddColor;
