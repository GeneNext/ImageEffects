...
public class Reflection : MonoBehaviour
{
    ...
    private RenderTexture m_reflectTexture = null;

    void OnEnable()
    {
        if (m_reflectTexture == null)
        {
            m_reflectTexture = RenderTexture.GetTemporary(Screen.width, Screen.height, 16, RenderTextureFormat.RGB565);
        }

        m_reflectCamera.CopyFrom(m_mainCamera);

        m_reflectCamera.clearFlags = CameraClearFlags.Skybox;
        ...
        m_renderer.sharedMaterial.SetTexture("_ReflectTex", m_reflectTexture);
    }
