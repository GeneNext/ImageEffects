...
inline float4 UnitySpecularLight(SpecularOutput s, half3 viewDir, UnityLight light)
{
	...
	float3 rimColor = rim * _RimColor * diffColor;

	float nh = max(0, dot(s.Normal, h)); 
	float3 specColor = saturate(pow(nh, s.Specular * 128.0) * s.Gloss) * diffColor;

	float4 outputColor;
	outputColor.rgb = s.Albedo * light.color * (diffColor + rimColor + specColor);
